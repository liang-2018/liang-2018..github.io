...
```bash
# 在进入 redis-cli 后
config get dir # 获取配置文件路径
config get requirepass # 获取密码
```

+ 非docker情况下，配置文件在安装目录内， redis.conf
+ docker 容器中默认没有配置文件，需要自己创建,可以通过 redis-server 自定配置文件路径
...
```bash
# 修改 redis.conf 文件
bind 127.0.0.1 # 绑定 localhost ip，注释掉可去绑定,也可改为 * 匹配所有。
protected-mode yes # 保护模式，该为 no
```
...
|       指令       |      含义      |                             示例                             |                            备注                             |
| :--------------: | :------------: | :----------------------------------------------------------: | :---------------------------------------------------------: |
|       zadd       | 添加有序键值对 |     zadd key score member<br />zadd liang 25 age 2 grade     |                      返回添加成功个数                       |
|      zrange      |      查看      |                   zrange 0 -1 [withscores]                   |                      liang 23 grade 2                       |
|  zrangebyscore   |  按照分数排序  | zrangebyscore key min max [witchscores limit offset count]<br />zrangebyscore key (60 90 | 如果没有 (, 不包含该分数，<br />添加 ( 后包含该值，即闭区间 |
|       zrem       |      删除      |                     zrem key member.....                     |                                                             |
|      zcard       |    统计个数    |                          zcard key                           |                                                             |
|      zcount      |    条件统计    |                      zcount key min max                      |                                                             |
|      zrank       |    获取下标    |                       zrank key member                       |                                                             |
|      zscore      |   查看score    |                       zcore key member                       |                                                             |
|     zrevrank     |    逆序下标    |                     zrevrank key member                      |                                                             |
|    zrevrange     |    逆序获取    |                   zrevrange key start stop                   |                                                             |
| zrevrangebyscore |    逆序排序    |             zrevrangebyscore key max min [....]              |                     与zrangebyscore同理                     |
...
################################## INCLUDES ###################################
# 可以包含一个或多个 redis 配置文件；可以分模块配置，方便维护
...
save 900 1 # 900秒以内修改 1次
save 300 10 # 5分钟内修改 10次
save 60 10000 # 60秒修改 10000次
